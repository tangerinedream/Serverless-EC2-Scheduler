...
class Loader( object ):
  ...
  def isRequiredSequencing( self ):
    ...
    if (isDifferent):
      logger.error(
        'Error: Start and Stop tiers contain different indexes. Here are the differences: mismatched TierSequences %s, Start TierSequences %s, Stop TierSequences %s' % (
        symmetricDifferenceOfTierIndexLists, startTierIndexList, stopTierIndexList) )
      return (False)
    ...
    if (maxIdx != len( startSet ) - 1):
      logger.error(
        'Error: Tier Start and Tier Stop TierSequences must be sequential starting at zero and without gaps, nor duplicates. Index List is %s' % startTierIndexList )
      return (False)
  ...
  def deleteTiers( self ):
    ...
    response = tiersTable.query(
      KeyConditionExpression=Key( Loader.TIER_PARTITION_KEY ).eq( firstTier[Loader.TIER_PARTITION_KEY] ) )
    ...
    for aTier in theTiers:
      primaryKey = {Loader.TIER_PARTITION_KEY: aTier[Loader.TIER_PARTITION_KEY],
                    Loader.TIER_SORT_KEY: aTier[Loader.TIER_SORT_KEY]}
      logger.info(
        "Deleting Tier {%s, %s}" % (primaryKey[Loader.TIER_PARTITION_KEY], primaryKey[Loader.TIER_SORT_KEY]) )
  ...
  def loadTiers( self ):
    ...
    for aTier in theTiers:
      logger.info(
        "Loading Tier {%s, %s}" % (aTier.get( Loader.TIER_PARTITION_KEY ), aTier.get( Loader.TIER_SORT_KEY )) )
...
if __name__ == "__main__":
  ...
  parser.add_argument( '-v', '--validateOnly', help='Only verify the Yaml file, do not execute any changes',
                       action="store_true", required=False )
  parser.add_argument( '-l', '--logLevel', choices=['critical', 'error', 'warning', 'info', 'debug', 'notset'],
                       help='The level to log', required=False )
